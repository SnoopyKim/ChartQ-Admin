...
// 스터디별 상세 통계 조회
export async function getStudyStatsDetailed(): Promise<StudyAnalyticsResponse<StudyStatsDetailed[]>> {
  try {
...
// 특정 스터디의 상세 통계 조회
export async function getStudyStatsById(studyId: string): Promise<StudyAnalyticsResponse<StudyStatsDetailed>> {
  try {
    const { data, error } = await supabase
      .from('study_stats_detailed')
      .select('*')
...
// 사용자 타입별 통계 조회
export async function getUserTypeStats(): Promise<StudyAnalyticsResponse<UserTypeStats[]>> {
  try {
    const { data, error } = await supabase
...
// 월별 통계 조회
export async function getMonthlyStats(limit: number = 12): Promise<StudyAnalyticsResponse<MonthlyStats[]>> {
  try {
    const { data, error } = await supabase
      .from('monthly_stats')
      .select('*')
...
export async function getPopularStudies(): Promise<StudyAnalyticsResponse<PopularStudy[]>> {
  try {
    const { data, error } = await supabase
...
// 스터디별 조회수 (Flutter 앱용)
export async function getStudyViewCounts(): Promise<StudyAnalyticsResponse<StudyViewCount[]>> {
  try {
    const { data, error } = await supabase
...
export async function getDashboardStats(): Promise<StudyAnalyticsResponse<DashboardStats>> {
  try {
    // 병렬로 여러 통계 데이터를 가져옴
    const [
      studyStatsResult,
      ...
      getPopularStudies(),
      getDailyStudyStats(
        new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 7일 전
    ...
    if (popularStudiesResult.error) return { error: popularStudiesResult.error };
    ...
    // 통합 통계 계산
    const totalViews = studyStats.reduce((sum, study) => sum + study.total_views, 0);
    const totalUniqueUsers = Math.max(...studyStats.map(study => study.unique_users), 0);
    ...
    const viewsLast7Days = studyStats.reduce((sum, study) => sum + study.views_last_7_days, 0);
    const viewsLast30Days = studyStats.reduce((sum, study) => sum + study.views_last_30_days, 0);
...
export async function getStatsSummary(
  ...
  try {
    ...
    const topStudyId = Object.entries(studyViewCounts)
